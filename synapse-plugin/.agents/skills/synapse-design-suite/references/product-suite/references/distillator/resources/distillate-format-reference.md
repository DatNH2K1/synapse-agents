...
platforms with no shared methodology layer. Synapse is uniquely positioned to be the
...
building on the open-source skills CLI ecosystem, Synapse offloads the highest-churn
...
```
## Differentiation
- Anti-fragmentation positioning: Synapse = cross-platform constant across 40+ fragmenting AI tools; no competitor provides shared methodology layer
- Platform complexity delegated to Vercel skills CLI ecosystem (MIT); Synapse maintains methodology, not platform configs
```
...
- **Key gap**: NO competitor combines structured methodology with plugin
  marketplace — this is Synapse's whitespace
```
...
type: synapse-distillate
sources:
  - "product-brief-synapse-next-gen-installer.md"
  - "product-brief-synapse-next-gen-installer-discovery-notes.md"
downstream_consumer: "PRD creation"
created: "2026-03-13"
token_estimate: 1450
parts: 1
...
- Synapse Next-Gen Installer: replaces monolithic Node.js CLI with skill-based plugin architecture for distributing Synapse methodology across 40+ AI platforms
- Three layers: self-describing plugins (.env), cross-platform install via Vercel skills CLI (MIT), runtime registration via synapse-setup skill
- Transforms Synapse from dev-only methodology into open platform for any domain (creative, therapeutic, educational, personal)
...
- Plugins: skill bundles with Anthropic plugin standard as base format + .env extending for SYNAPSE-specific metadata (installer options, capabilities, help integration, phase ordering, dependencies)
- Existing manifest example: `{"module-code":"synapse","replaces-skill":"synapse-create-product-brief","capabilities":[{"name":"create-brief","menu-code":"CB","supports-headless":true,"phase-name":"1-analysis","after":["brainstorming"],"before":["create-prd"],"is-required":true}]}`
- Vercel skills CLI handles platform translation; integration pattern (wrap/fork/call) is PRD decision
- synapse-setup: global skill scanning installed .env files, registering capabilities, configuring project settings; always included as base skill in every bundle (solves bootstrapping)
- synapse-update: plugin update path without full reinstall; technical approach (diff/replace/preserve customizations) is PRD decision
...
- Anti-fragmentation: Synapse = cross-platform constant; no competitor provides shared methodology layer across AI tools
- Curated quality: all submissions gated, human-reviewed by Synapse + core team; 13.4% of community skills have critical vulnerabilities (Snyk 2026); quality gate value increases as ecosystem gets noisier
- Domain-agnostic: no competitor builds beyond software dev workflows; same plugin system powers any domain via Synapse Builder (separate initiative)
...
- Future plugin creators: non-dev authors using Synapse Builder; need distribution without building own installer
...
- synapse-setup discovers/registers all plugins from manifests; clear errors for malformed manifests
- At least one external module author successfully publishes plugin using manifest system
- synapse-update works without full reinstall
- Existing CLI users have documented migration path

## Scope

- In: manifest spec, synapse-setup, synapse-update, Vercel CLI integration, NPX installer, zip bundles, migration path
- Out: Synapse Builder, marketplace web platform, skill conversion (prerequisite, separate), one-click install for all platforms, monetization, quality certification process (gated-submission principle is architectural requirement; process defined separately)
...
- Entry: `tools/installer/synapse-cli.js` (Commander.js) → `tools/installer/core/installer.js`
...
- Skills already use directory-per-skill layout; .env sidecars exist but are not source of truth
...
- Discovery: `skills/`, `.agents/skills/`, agent-specific paths, `.agent-plugin/marketplace.json`
...
- synapse-update mechanics: diff/replace? Preserve user customizations?
...
- .env as open standard submission to Agent Skills governance?
...
- Module authors as acquisition channel: each published plugin distributes Synapse to creator's audience
- CI/CD integration: synapse-setup as pipeline one-liner increases stickiness
- Educational institutions: structured methodology + non-technical install → university AI curriculum
- Skill composability: mixing Synapse modules with third-party skills for custom methodology stacks
