...
nextStepFile: "[File references (ONLY variables used in this step)](./step-v-03-density-validation.md)"
altStepFile: "[File references (ONLY variables used in this step)](./step-v-02b-parity-check.md)"
...
Detect if PRD follows Synapse format and route appropriately - classify as Synapse Standard / Synapse Variant / Non-Standard, with optional parity check for non-standard formats.
...
### 2. Check for Synapse PRD Core Sections

Check if the PRD contains the following Synapse PRD core sections:
...
- Follows Synapse PRD structure closely

**Synapse Variant:**

- 3-4 core sections present
- Generally follows Synapse patterns but may have structural differences
- Missing some sections but recognizable as SYNAPSE-style

**Non-Standard:**

- Fewer than 3 core sections present
- Does not follow Synapse PRD structure
...
**Format Classification:** [Synapse Standard / Synapse Variant / Non-Standard]
**Core Sections Present:** [count]/6
```

### 5. Route Based on Format Classification

**IF format is Synapse Standard or Synapse Variant:**
...
This PRD does not follow Synapse standard structure (only {count}/6 core sections present).
...
**[A] Parity Check** - Analyze gaps and estimate effort to reach Synapse PRD parity
...
- Synapse core sections checked systematically
- Format classified correctly based on section count
- Findings reported to validation report
- Synapse Standard/Variant PRDs proceed directly to next validation step
