...
**All workflows**: Activate `synapse-design-suite` skill FIRST for design intelligence.

**Precedence:** When anti-slop rules (below) conflict with `synapse-design-suite` recommendations (e.g., Inter font, AI Purple palette, Lucide-only icons), substitute with alternatives from `./references/anti-slop-rules.md` unless the user explicitly requested the conflicting choice.

## Screenshot/Video Replication (Quick Reference)

1. **Analyze** with `synapse-ai-multimodal` skill - extract colors, fonts, spacing, effects
...
**Assets**: Generate images with `synapse-ai-multimodal`, process with `synapse-media-processing`
...
**Quick chesynapse-** See the "AI Tells" checklist in `./references/anti-slop-rules.md` before delivering any design.
