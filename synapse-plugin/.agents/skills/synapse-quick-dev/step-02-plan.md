...
3. Read [Spec Template](./spec-template.md) fully. Fill it out based on the intent and investigation. If `{preserved_intent}` is non-empty, substitute it for the `<frozen-after-approval>` block in your filled spec before writing. Write the result to `{spec_file}`.
...
Before approving, you can open the spec file in an editor or ask me questions and tell me what to change. You can also use `synapse-advanced-elicitation`, `synapse-party-mode`, or `synapse-code-review` skills, ideally in another session to avoid context bloat.
...
Read fully and follow [Step 3: Implement](./step-03-implement.md)
