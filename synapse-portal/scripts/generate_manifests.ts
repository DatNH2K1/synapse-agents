...
function main() {
  ...
  if (fs.existsSync(skillsDir)) {
    ...
    for (const skillPath of skillPaths) {
      ...
      const relativeFolder = path
        .relative(skillsDir, path.dirname(skillPath))
        .replace(/\\/g, "/");
  ...
  fs.writeFileSync(
    path.join(manifestsDir, "skill-manifest.csv"),
    skillRecords.join("\n") + "\n",
    "utf-8",
  );
  console.log(
    `Generated skill-manifest.csv with ${skillRecords.length - 1} entries.`,
  );
  ...
  fs.writeFileSync(
    path.join(manifestsDir, "agent-manifest.csv"),
    agentRecords.join("\n") + "\n",
    "utf-8",
  );
  console.log(
    `Generated agent-manifest.csv with ${agentRecords.length - 1} entries.`,
  );
  ...
  fs.writeFileSync(
    path.join(manifestsDir, "tool-manifest.csv"),
    toolRecords.join("\n") + "\n",
    "utf-8",
  );
  console.log(
    `Generated tool-manifest.csv with ${toolRecords.length - 1} entries.`,
  );

  // ==========================================
  // 4. Generate personal-manifest.csv
  // ==========================================
  const personalsCsvPath = path.join(
    workspaceRoot,
    "synapse-plugin",
    ".agents",
    "agents",
  ...
  const personalRecords: string[] = [
    "id,displayName,region,description,cultural_traits,tech_literacy,pain_points",
  ];
  ...
  fs.writeFileSync(
    path.join(manifestsDir, "personal-manifest.csv"),
    personalRecords.join("\n") + "\n",
    "utf-8",
  );
  console.log(
    `Generated personal-manifest.csv with ${personalRecords.length - 1} entries.`,
  );
}
