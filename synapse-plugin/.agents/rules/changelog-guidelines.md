...
- **No Absolute Paths (CRITICAL)**: Do not write absolute file paths (e.g., `file:///absolute/path/to/file` or absolute paths starting with `/Users/...`) into any `CHANGELOG.md` files or git commit messages. Always use relative paths (e.g. `src/constants/types.ts`) or file basenames. Note: Even though agent instructions mandate using absolute file URLs in chat responses to the USER, you MUST NOT write them into the repository files.
