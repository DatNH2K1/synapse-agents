...
- IF A: Invoke the `synapse-advanced-elicitation` skill with the current FR list, process the enhanced capability coverage that comes back, ask user if they accept the additions, if yes update content then redisplay menu, if no keep original content then redisplay menu
- IF P: Invoke the `synapse-party-mode` skill with the current FR list, process the collaborative capability validation and additions, ask user if they accept the changes, if yes update content then redisplay menu, if no keep original content then redisplay menu
- IF C: Append the final content to {outputFile}, update frontmatter by adding this step name to the end of the stepsCompleted array, then read fully and follow: [Step 10: Non-Functional Requirements](./step-10-nonfunctional.md)
...
After user selects 'C' and content is saved to document, load [Step 10: Non-Functional Requirements](./step-10-nonfunctional.md) to define non-functional requirements.
