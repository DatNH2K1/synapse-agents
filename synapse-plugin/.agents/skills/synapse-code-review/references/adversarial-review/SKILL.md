...
description: "Review code quality with adversarial rigor. Supports three-stage protocol: Spec Compliance, Code Quality, and Adversarial Red-teaming."
...
| **Checklist review**     | Pre-landing, `/synapse-ship` pipeline, security audit          | `references/checklist-workflow.md`             |
...
1. Invoke `/synapse-scout` with edge-case-focused prompt
...
**Fallbasynapse-** Task tools (`TaskCreate`/`TaskUpdate`/`TaskGet`/`TaskList`) are CLI-only — unavailable in VSCode extension. If they error, use `TodoWrite` for tracking and run pipeline sequentially. Review quality is identical.
...
| --------------------------------------- | ---------------------------------------- | ------------------------------------------- |
| `synapse-code-review codebase`          | `references/codebase-scan-workflow.md`   | Scan & analyze the codebase                 |
| `synapse-code-review codebase parallel` | `references/parallel-review-workflow.md` | Ultrathink edge cases, then parallel verify |
