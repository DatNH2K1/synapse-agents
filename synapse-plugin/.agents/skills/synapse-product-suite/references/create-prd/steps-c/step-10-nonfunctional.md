...
- IF A: Invoke the `synapse-advanced-elicitation` skill with the current NFR content, process the enhanced quality attribute insights that come back, ask user if they accept the improvements, if yes update content then redisplay menu, if no keep original content then redisplay menu
- IF P: Invoke the `synapse-party-mode` skill with the current NFR list, process the collaborative technical validation and additions, ask user if they accept the changes, if yes update content then redisplay menu, if no keep original content then redisplay menu
- IF C: Append the final content to {outputFile}, update frontmatter by adding this step name to the end of the stepsCompleted array, then read fully and follow: [Step 11: Document Polish](./step-11-polish.md)
...
After user selects 'C' and content is saved to document, load [Step 11: Document Polish](./step-11-polish.md) to finalize the PRD and complete the workflow.
