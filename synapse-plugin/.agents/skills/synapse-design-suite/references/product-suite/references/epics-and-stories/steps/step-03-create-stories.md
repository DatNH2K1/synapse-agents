...
- IF A: Invoke the `synapse-advanced-elicitation` skill
- IF P: Invoke the `synapse-party-mode` skill
- IF C: Save content to {planning_artifacts}/epics.md, update frontmatter, then read fully and follow: [Step 4: Final Validation](./step-04-final-validation.md)
...
ONLY WHEN [C continue option] is selected and [all epics and stories saved to document following the template structure exactly], will you then read fully and follow: [Step 4: Final Validation](./step-04-final-validation.md) to begin final validation phase.
