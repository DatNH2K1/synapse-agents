...
- IF A: Invoke the `synapse-advanced-elicitation` skill with the current project-type content, process the enhanced technical insights that come back, ask user "Accept these improvements to the technical requirements? (y/n)", if yes update content with improvements then redisplay menu, if no keep original content then redisplay menu
- IF P: Invoke the `synapse-party-mode` skill with the current project-type requirements, process the collaborative technical expertise and validation, ask user "Accept these changes to the technical requirements? (y/n)", if yes update content with improvements then redisplay menu, if no keep original content then redisplay menu
- IF C: Append the final content to {outputFile}, update frontmatter by adding this step name to the end of the stepsCompleted array, then read fully and follow: [Step 8: Scoping Exercise - MVP & Future Features](./step-08-scoping.md)
...
After user selects 'C' and content is saved to document, load [Step 8: Scoping Exercise - MVP & Future Features](./step-08-scoping.md) to define project scope.
