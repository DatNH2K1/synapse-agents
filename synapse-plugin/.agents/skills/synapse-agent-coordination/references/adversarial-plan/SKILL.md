---
name: synapse-adversarial-plan
description: |
  Create implementation plans with adversarial rigor. Includes red-team analysis, risk assessment, and secondary effect analysis.

  MANDATORY: Run this skill whenever creating a new technical implementation plan, architecting system designs, or preparing task checklists for execution in the project.

  Trigger immediately for:
  - Creating new feature plans, roadmaps, or architecture designs.
  - Doing risk assessment or hostile peer reviews on technical approaches.
  - Standard plan actions like `/synapse-plan`, `adversarial plan`, `red-team review`, `validate plan`, or `archive plan`.

  DO NOT trigger for:
  - Simple code fixes, styling tweaks, minor debugging, or editing non-architectural code.
  - Answering general questions about the codebase without needing a plan.
...
3. **Classify relationship:**
   ...
   - New plan changes something existing plan depends on → existing plan `blockedBy: [new-plan-dir]`, new plan `blocks: [existing-plan-dir]`, new plan `blocks: [existing-plan-dir]`
...
Ref: `workflow-modes.md` for mode transitions and rules.
...
6. **Red Team Review** → Run `/synapse-plan red-team {plan-path}` (hard/parallel/two modes)
7. **Post-Plan Validation** → Run `/synapse-plan validate {plan-path}` (hard/parallel/two modes)
...
After creating plan: `node scripts/set-active-plan.cjs {plan-dir}`
