...
- When 'A' selected: Invoke the `synapse-advanced-elicitation` skill
- When 'P' selected: Invoke the `synapse-party-mode` skill
...
- Invoke the `synapse-advanced-elicitation` skill with current category rules
...
- Invoke the `synapse-party-mode` skill with category rules context
...
When user selects 'C' for a category, append the content directly to `project-context.md` using the structure from step 8.
...
After completing all rule categories and user selects 'C' for the final category, load [Step 3: Context Completion & Finalization](./step-03-complete.md) to finalize the project context file.
